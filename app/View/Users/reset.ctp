...
$cakeDescription = __d('cake_dev', 'ImmoEasy.net: neues Passwort erstellen');
...
//    ));?>
        ...
        <h2 class="form-signin-heading">Neues Passwort eingeben</h2>

        <p>
            Geben Sie hier Ihr neues Passwort ein.
            <?php
            echo $this->Html->link(
                'Sie haben bereits ein Passwort. Hier anmelden?',
                array(
                    'controller' => 'users',
                    'action' => 'login',
                    'full_base' => true,
...
<!--            </form>-->
        <form method="post">
            <?php
            echo $this->Form->input('password',array("type"=>"password","name"=>"data[User][password]",'class' => 'form-control','label' => 'Passwort'));
            echo $this->Form->input('passwordConfirm',array("type"=>"password","name"=>"data[User][passwordConfirm]",'class' => 'form-control','label' => 'Passwort erneut eingeben'));
            ?></br>
            <input type="submit" class="btn btn-lg btn-primary btn-block" style="float:left;margin-left:3px;" value="Speichern"  />
